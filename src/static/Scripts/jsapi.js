...
if (!window['google']['loader']) {
	...
	(function() {
		var d =
		void 0, g = !0, h = null, k = !1, l = encodeURIComponent, m = window, n = document;
		function p(a, b) {
			return a.load = b
		}
		...
		function L(a, b) {
			for (var c = a.split(/\./), e = m, f = 0; f < c[z] - 1; f++)
				e[c[f]] || (e[c[f]] = {}), e = e[c[f]];
			e[c[c[z] - 1]] = b
		}
		...
		p(google, function(a, b, c) {
			...
			if ( a = O[":" + f]) {
				...
				c && "string" == typeof c.callback && ( j = c.callback, j.match(/^[[\]A-Za-z0-9._]+$/) && ( j = m.eval(j), c.callback = j));
				if (( j = c && c.callback != h) && !a.s(b))
					throw K("Module: '" + f + "' must be loaded before DOM onLoad!");
				j ? a.m(b, c) ? m[B](c.callback, 0) : a.load(b, c) : a.m(b, c) || a.load(b, c)
			} else
				throw K("Module: '" + f + "' not found!");
		});
		M("google.load", google.load);
		google.S = function(a, b) {
			b ? (0 == Q[z] && (R(m, "load", S), !G("msie") && !G("safari") && !G("konqueror") && G("mozilla") || m.opera ? m.addEventListener("DOMContentLoaded", S, k) : G("msie") ? n.write("<script defer onreadystatechange='google.loader.domReady()' src=//:>\x3c/script>") : (G("safari") || G("konqueror")) && m[B](ba, 10)), Q[r](a)) : R(m, "load", a)
		};
		...
		google[C].d = function(a, b, c) {
			if (c) {
				...
				"script" == a ? n.write('<script src="' + b + '" type="text/javascript">\x3c/script>') : "css" == a && n.write('<link href="' + b + '" type="text/css" rel="stylesheet"></link>')
		};
		...
		V[A].t = function(a) {
			...
			if (b)
				if ("string" == typeof b)
					...
					c = [];
					for ( a = 0; a < b[z]; a++)
						"string" == typeof b[a] && c[r](b[a][s](/^\s*|\s*$/,"")[F]())
				}
			c || ( c = ["default"]);
			b = [];
			for ( a = 0; a < c[z]; a++)
		...
		p(V[A], function(a, b) {
			...
			if (c[z]) {
				...
				if (!b && P[":" + this.b] != h && P[":"+this.b].versions[":" + a] != h && !google[C].AdditionalParams && this.j) {
					...
					(!b || !b.autoloaded) && google[C].d("script", this.g(a, b), e);
		...
		T[A].g = function(a, b) {
			if (!this.i.versions[":" + a]) {
				...
				if (!this.i.versions[":" + a])
					throw K("Module: '" + this.b + "' with version '" + a + "' not found!");
			}
		...
		var ea = k, Z = [], fa = (new Date)[y](), ha = function() {
			...
			if (!google[C].Secure && (!google[C].Options || google[C].Options.csi === k)) {
				for (var c = 0; c < a[z]; c++)
					a[c] = l(a[c][F]()[s](/[^a-z0-9_.]+/g, "_"));
				for ( c = 0; c < b[z]; c++)
					b[c] = l(b[c][F]()[s](/[^a-z0-9_.]+/g, "_"));
